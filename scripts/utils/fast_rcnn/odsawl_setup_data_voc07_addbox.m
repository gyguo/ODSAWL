function imdb = odsawl_setup_data_voc07_addbox(reswsddn,imdbwsddn,imdbFastRcnn,varargin)
...
for thisSet = {'train', 'val', 'test'}
  ...
  for i=1:length(gtids)
    ...
    %set gt box for train and val
    if k==1 | k==2
   
        curImg = cell2mat(gtids(i));
        curImg = str2num(curImg);
        curIdx = find(trainIdx == curImg);
        curgt = reswsddn(curIdx);
        curgt = curgt{1};
		
		BB = [];
		if curgt.istrain
			label = unique(curgt.cats);
            for cls = 1:numel(label)
                curboxesIdx = find(curgt.cats==label(cls));
                [~,maxPos] = max(curgt.confidence(curboxesIdx));
                BB = [BB;curgt.bbox(curboxesIdx(maxPos),:)];
            end
		
		else
			[~,maxPos] = max(curgt.confidence);
			BB = curgt.bbox(maxPos,:);
			label = curgt.cats(maxPos);
		end
