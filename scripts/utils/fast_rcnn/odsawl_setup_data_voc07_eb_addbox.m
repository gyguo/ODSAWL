function imdb = odsawl_setup_data_voc07_eb_addbox(restrainvalwsddn,imdbwsddn,imdbFastRcnn,varargin)
%CNN_SETUP_DATE_VOC07_eb  Setup PASCAL VOC 2007 data with precomputed eb proposals
%
...
%% Get edge boxes
files = {...
  'EdgeBoxesVOC2007trainval.mat', ...
  'EdgeBoxesVOC2007test.mat'} ;

  
%% Get image names and gt boxes.
imdb = odsawl_setup_data_voc07_addbox(restrainvalwsddn,...
        imdbwsddn,imdbFastRcnn,...
        'dataDir', opts.dataDir, ...
        'useDifficult', opts.useDifficult, ...
        'addFlipped', opts.addFlipped) ;

%% Load precomputed object proposals
train = load(fullfile(opts.ebDir, files{1})) ;
test = load(fullfile(opts.ebDir, files{2})) ;
